...
This is not just a teaching curiosity. The same reasoning, strain stays linear and continuous, stress follows the material, and the axis is located by equilibrium, is exactly what you need to reason correctly about any section built from parts that carry load differently. Composite and partially composite systems, mixed-material members, anything where one component takes compression and another takes tension across an interface: they all yield to the same first-principles thinking. Get the cracked concrete case truly clear, and a surprising amount of harder structural behavior stops being mysterious.
